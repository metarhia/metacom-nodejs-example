  ...
  const introspect = (units) => {
    ...
    for (const unitName of units) {
      ...
      for (const [key, fn] of methods) {
        if (key.startsWith(prefix) && typeof fn === 'function') {
          unitMethods[key.slice(prefix.length)] = {};
        }
      }
      if (Object.keys(unitMethods).length > 0) result[unitName] = unitMethods;
    }
    return result;
  };
    ...
    getMethod(unit, ver, method) {
      if (unit === 'system' && method === 'introspect') {
        return {
          access: 'public',
          enter: async () => {},
          invoke: (context, units = []) => introspect(units),
          leave: () => {},
        };
      }
